...
| 訊息推送 | Redis → MCP server: long-poll (XREADGROUP BLOCK)；MCP server → Claude Code: push (SSE notification) | Claude Code `server:` 設定可接收 MCP 推送 |
...
配對只在**私訊**中進行（group 中 `/start` 不觸發配對）。每個 bot 只能綁定一個用戶。

```
用戶私訊 bot: /start
      │
      ▼
已配對？─── YES → 回覆「已配對，你可以開始使用」
      │
      NO
      │
      ▼
產生 6 位配對碼，存入 Redis pairing:{agent_id}:code (TTL 60s)
Bot 回覆：「你的配對碼是 482901」+ [確認配對] inline button
      │
      ▼
用戶點擊 inline button
      │
      ▼
Bot 驗證碼未過期 → 綁定 agent:{agent_id}:paired_user = ctx.from.id
回覆「配對成功 ✓」
      │
      ▼
若碼已過期 → 回覆「配對碼已過期，請重新 /start」
```
